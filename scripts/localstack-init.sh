...
# Cognito setup (LocalStack Pro only)
# Check if LOCALSTACK_API_KEY is set (indicates Pro license)
if [ -n "$LOCALSTACK_API_KEY" ]; then
    echo ""
    echo "🔐 LocalStack Pro detected - Setting up Cognito..."
...
Test user: test@example.com / TestPass123!"
    ...
    echo ""
    echo "ℹ️  Cognito setup skipped (LocalStack Pro required)"
    echo "   Set LOCALSTACK_API_KEY to enable Cognito"
    COGNITO_OUTPUT="
Cognito: Skipped (requires LocalStack Pro)
   To enable: export LOCALSTACK_API_KEY=your-key"
fi
